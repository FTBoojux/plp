# Netty开发进阶：使用 EmbeddedChannel 测试TLV协议中的半包、粘包等情况
> 💡 **本文源码说明**
>
> 本文所有演示代码均已开源。
> *   **代码仓库**：[https://github.com/FTBoojux/plp]
> *   **本章模块**：`example/EmbeddedChannelInNetty`
>
> 建议您将代码拉取到本地，直接运行 `MyDecoderTest` 中的测试用例，体验更加直观。

## 1.前言：什么是TLV协议

在本节中，我们通过自定义TLV协议解码器，进一步演示`EmbeddedChannel`在Netty开发中的使用。

### 1.1 痛点分析：半包和粘包

半包和粘包是通过网络进行数据传输时的常见问题。
- 半包：一个数据包被分为多个数据包进行发送。常由原始数据包过大、写入速度过快、窗口过小等原因导致。
- 粘包：多个数据包被合并为一个数据包进行发送。常由发送方为提高效率使用Nagle算法合并过小的数据包、接收方处理和读取数据过慢等原因导致。

半包和粘包的根本原因：TCP协议将数据看作无边界的字节流，传输时并不区分数据包的边界。

### 1.2 解决方案：TLV协议
类型-长度-值(Type/Tag-Length-Value)协议，简称TLV协议，是常用的用于解决半包和粘包情况的解决方案。它将数据组织为以下三部分：
- 类型(Type/Tag): 一个用于区分数据类型的识别码。
- 长度(Length)：一个用于指定数据长度的数字，通常由载荷大小决定。
- 值(Value)：实际传输的数据

## 2.实战

### 2.1 数据结构

首先我们定义自己的TLV消息结构体如下：
- Type: 表示数据类型，1 byte
- Length: 表示数据长度，4 bytes
- Value：实际的数据，长度由Length决定

```java
public record TlvMessage<T>(byte type,int length,T value) {
}
```

### 2.2 定义自己的TLV解码器

我们确定如下的TLV解码逻辑：
- 1. 检查当前可读字段是否 >= 5 (type + length)。如果不满足，则返回等待后续数据。
- 2. 标记当前 readerIndex
- 3. 读取 type
- 4. 读取 length
- 5.检查剩余可读字节数是否 >= length
- 6.如果不满足：
  - 重置 readerIndex，并返回等待更多数据
- 7.若满足：
  - 读取长度为 length 的字节数据，作为 value

本实验中其实现如下：
```java
public class MyDecoder extends ByteToMessageDecoder {
    @Override
    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
        //  检查可读字节是否 >= 5 (Type + Length)。如果不满足，返回 (等待更多数据)
        while(byteBuf.readableBytes() >= 5) {
            // 标记当前 readerIndex。
            int readIndex = byteBuf.readerIndex();
            // 读取type和length
            byte type = byteBuf.readByte();
            ByteBuf lengthBuf = byteBuf.readBytes(4);
            int length = lengthBuf.readInt();
            lengthBuf.release();
            // 检查可读字节是否 >= Length
            if(byteBuf.readableBytes() < length) {
                // 重置 readerIndex 到标记位置 (回滚读取指针)
                byteBuf.readerIndex(readIndex);
                return;
            } else {
                // 读取 Value (长度为 Length 的字节),并将解析的对象输出
                // 本实验中仅使用String类型的数据
                ByteBuf informationBuf = byteBuf.readBytes(length);
                String information = informationBuf.toString(StandardCharsets.UTF_8);
                TlvMessage<String> tlvMessage = new TlvMessage<>(type, length, information);
                informationBuf.release();
                list.add(tlvMessage);
            }
        }
    }
}
```

同时，我们用一个简单的`StringEchoHandler`来返回数据：
```java
public class StringEchoHandler extends ChannelInboundHandlerAdapter {
    @Override
    public void channelRead(ChannelHandlerContext ctx, Object msg) {
        TlvMessage<String> message = (TlvMessage<String>)msg;
        ctx.write(message.value());
        ctx.flush();
    }
}
```

### 2.3 工具方法

我们定义如下用于创建测试时使用的`EmbeddedChannel`和数据包的工具方法：
```java
public class MyDecoderTest {
    private static @NonNull EmbeddedChannel getEmbeddedChannel() {
        return new EmbeddedChannel(
                new MyDecoder(),
                new StringEchoHandler()
        );
    }
    private static ByteBuf encodeFrame(String s) {
        byte[] bytes = s.getBytes(CharsetUtil.UTF_8);
        ByteBuf buf = Unpooled.buffer(5 + bytes.length);
        // 本实验中1表示字符串类型
        buf.writeByte(1);
        buf.writeInt(bytes.length);
        buf.writeBytes(bytes);
        return buf;
    }
}
```

### 2.4 测试用例

#### 2.4.1 正常情况(完整包)

```java
public class MyDecoderTest {
    @Test
    public void normal_package_should_echo(){
        ByteBuf hello = encodeFrame("hello");
        EmbeddedChannel channel = getEmbeddedChannel();
        channel.writeInbound(Unpooled.wrappedBuffer(hello));
        Object out = channel.readOutbound();
        Assertions.assertEquals("hello",out);
    }
}
```

#### 2.4.2 粘包

我们将两个数据包合并发送，并验证`MyDecoder`可以正确的将其拆分：
```java
public class MyDecoderTest {
    @Test
    public void sticky_packets_should_echo() {
        ByteBuf hello = encodeFrame("hello");
        ByteBuf world = encodeFrame("world");
        ByteBuf sticky_packets = Unpooled.wrappedBuffer(hello, world);
        EmbeddedChannel channel = getEmbeddedChannel();
        channel.writeInbound(sticky_packets);
    
        Object helloResponse = channel.readOutbound();
        Assertions.assertEquals("hello", helloResponse, "应当返回'hello'");
    
        Object worldResponse = channel.readOutbound();
        Assertions.assertEquals("world", worldResponse, "应当返回'world'");
    
        Assertions.assertNull(channel.readOutbound());
    }
}
```

#### 2.4.3 半包(头部拆分)

我们将数据包在 Length 和 Value 的边界处分割，验证当出现半包时，`MyDecoder`可以正确等待后续的数据：
```java
public class MyDecoderTest {
    @Test
    public void split_header_should_echo() {
        ByteBuf hello = encodeFrame("hello");
        EmbeddedChannel embeddedChannel = getEmbeddedChannel();
        embeddedChannel.writeInbound(hello.copy(0, 5));
        embeddedChannel.writeInbound(hello.copy(5, hello.readableBytes() - 5));
        Object response = embeddedChannel.readOutbound();
        Assertions.assertEquals("hello", response, "应当返回'hello'");
    }
}
```

#### 2.4.4 半包(数据拆分)
我们将数据包在 Value 中间分割，验证当出现头部完整但数据不足时，`MyDecoder`可以正确等待完整数据：
```java
public class MyDecoderTest {
    @Test
    public void split_body_should_echo() {
        ByteBuf hello = encodeFrame("hello");
        EmbeddedChannel embeddedChannel = getEmbeddedChannel();
        embeddedChannel.writeInbound(hello.copy(0, 7));
        embeddedChannel.writeInbound(hello.copy(7, hello.readableBytes() - 7));
        Object response = embeddedChannel.readOutbound();
        Assertions.assertEquals("hello", response, "应当返回'hello'");
    }
}
```

#### 2.4.5 粘包+半包的混合场景

我们准备两个数据包(hello+world)，并按如下顺序写入数据包：
- 1.hello的前半部分
- 读取，此时应没有结果
- 2.hello的剩余部分+world的前半部分
- 读取，此时仅返回hello
- 3.world的剩余部分
- 读取，此时返回world

```java
public class MyDecoderTest {
    @Test
    public void complex_case_should_echo_separately() {
        ByteBuf hello = encodeFrame("hello");
        ByteBuf world = encodeFrame("world");
        EmbeddedChannel embeddedChannel = getEmbeddedChannel();
    
        embeddedChannel.writeInbound(hello.copy(0, 4));
        Object response = embeddedChannel.readOutbound();
        Assertions.assertNull(response);
    
        embeddedChannel.writeInbound(Unpooled.wrappedBuffer(hello.copy(4, hello.readableBytes() - 4), world.copy(0, 5)));
        Object helloResponse = embeddedChannel.readOutbound();
        Assertions.assertEquals("hello", helloResponse, "应当返回'hello'");
        Assertions.assertNull(embeddedChannel.readOutbound());

        embeddedChannel.writeInbound(world.copy(5, world.readableBytes() - 5));
        Object worldResponse = embeddedChannel.readOutbound();
        Assertions.assertEquals("world", worldResponse, "应当返回'world'");
    }
}
```

### 3.总结

在本文中，我们通过自定义TLV协议解码器，并通过若干不同场景的测试用例（半包、粘包等），在验证解码器功能的同时，进一步掌握了使用`EmbeddedChannel`进行单元测试的方法。