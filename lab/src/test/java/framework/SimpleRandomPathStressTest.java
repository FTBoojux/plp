...
public class SimpleRandomPathStressTest {
    private final String targetUrlRule;
    ...
    public SimpleRandomPathStressTest(String targetUrlRule, int requests, int threads) {
        this.targetUrlRule = targetUrlRule;
    ...
    private List<String> generateMockUrls() {
        List<String> mockUrls = new ArrayList<>(this.totalRequest);
        for (int i = 0; i < this.totalRequest; i++) {
            mockUrls.add(generateMockUrl());
        }
        return mockUrls;
    }

    private String generateMockUrl() {
        String[] split = this.targetUrlRule.split("/");
        Iterator<String> iterator = Arrays.asList(split).iterator();
        StringBuilder sb = new StringBuilder();
        while (iterator.hasNext()){
            String current = iterator.next();
            if(current.startsWith("{") && current.endsWith("}")){
                sb.append(StringUtils.randomString());
            }else{
                sb.append(current);
            }
            if(iterator.hasNext()){
    ...
    private long measureSingleRequest(){
        ...
        try {
            URL url = URI.create("http://127.0.0.1:8000" + this.generateMockUrl()).toURL();
